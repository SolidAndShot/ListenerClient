...
进入游戏后按 **K** 打开“监听器规则编辑器”。这是一个不依赖服务器箱子 GUI 的三栏 Screen：左侧按屏幕/键盘/鼠标/世界/状态分类并搜索、滚轮分页选择 FancyMenu 26.2 的全部 85 个 Listener provider；中间编辑规则 ID 与 `*_contains` 条件；右侧预览并循环选择客户端/服务端动作。每个目录项都会标明“客户端已实现”“目录可选，需扩展”或“FancyMenu 专属，需扩展”，不会把尚未实现的资源事件伪装成可用事件。
...
服务端可返回可见规则列表，客户端会缓存到编辑器状态；管理员可以在编辑器中载入、保存、测试和删除服务器规则。服务端拒绝编辑请求时不会覆盖本地草稿。
